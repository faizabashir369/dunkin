...
<input type="text"  required="" placeholder="Select time"  name="pickup_time" id="datetimepicker7" value="<?php echo $newtimestamp;?>"readonly="readonly" />
    ...
    <script>
       ...
       // M.AutoInit();
        ...
        var today = new Date(today.getTime() - (today.getTimezoneOffset() * 60000 ))
                    .toISOString()
                    .split("T")[0];
                  //  var dd = String(today.getDate()).padStart(2, '0');
        var tim=new Date(new Date().getTime());
        ...
        const da = new Date(localStorage.getItem("selectedDate"));
                //const ye = new Intl.DateTimeFormat('en', { year: 'numeric' }).format(d);
                const mo = new Intl.DateTimeFormat('en', { month: 'short' }).format(da);
                const dat = new Intl.DateTimeFormat('en', { day: '2-digit' }).format(da);
                 //alert(`${da} ${mo}`);
                    //localStorage.getItem("selectedDate")
                    if(localStorage.getItem("selectedDate")==`${dat} ${mo}`)
                    {
                        console.log("today");
                   ...
                   // document.getElementById("datetime").value=strTime+" "+ dd+" "+mon;
             ...
             //   var  milk=items.milk_option;
                ...
                //alert("Mmilk Options "+options.milkopt);
                    ...
                    row +=   '</div>';
                      ...
                      row +=    '<p class="item-price" id="'+i+'pprice">SR '+product_price*quantity+'</p>';
                      row +=   '</div>';
